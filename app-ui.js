...
/* ══ Page navigation ══ */
const PAGES = ['player','stream','library','dj'];
...
/* ════════════════════════════════════════════════
   PAGE 1 — STREAM HUB
════════════════════════════════════════════════ */
/* Phase 19: pre-seeded demo tracks and MP4 samples removed entirely —
   the app no longer ships a Video tab or placeholder streams. Radio now
   lives in its own Stream/Radio tab (phase19.js: StreamHub), and the DJ
   browser's "STREAMS" section is simply empty/hidden with nothing here. */
const PRESEEDED = [];
window.PRESEEDED_STREAMS = PRESEEDED;
...
function initSplash() {
  ...
  const enterApp = () => {
    ...
    localStorage.setItem('vz_page','player');
    if (typeof showPage === 'function') showPage('player');
    setTimeout(()=>splash.remove(),600);
  };
  splash.addEventListener('click', enterApp);
  setTimeout(enterApp, 6000); // safety fallback only — tap is the primary gate
}
...
document.addEventListener('DOMContentLoaded',()=>{
  initSplash();
  // Restore page
  const saved=localStorage.getItem('vz_page')||'player';
  showPage(saved);
  // Library events
