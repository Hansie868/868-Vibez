...
const DB_NAME = '868VibezDB', DB_VERSION = 5;
let _db = null;
...
const FULL_DB_SCHEMA = [
  ...
  'trackBlobs',                                                                // v5
  // v5: Android Chrome has no showDirectoryPicker() / File System Access
  // API — every import path in this app (openFolder, scanFolder) relied
  // on it exclusively, so on Android nothing could ever be imported and
  // therefore nothing could ever play. trackBlobs stores the actual
  // audio Blob directly (via a <input type="file"> picker instead of a
  // directory handle), so import — and therefore playback — works on
  // every device, not just desktop Chrome/Edge.
];
...
async function fileFromTrack(t) {
  if(t._fileHandle) return getFileWithPermission(t._fileHandle);
  const stored=await dbGet('handles',t.id);
  if(stored?.handle) { t._fileHandle=stored.handle; return getFileWithPermission(stored.handle); }
  // Android-safe fallback path: track was imported via <input type="file">
  // with no real handle available on that device, so its audio Blob was
  // copied directly into trackBlobs instead.
  const blobRec = await dbGet('trackBlobs', t.id);
  if (blobRec?.blob) return blobRec.blob;
  throw new Error('File not found — re-import this song or folder.');
}
/* A FileSystemFileHandle's read permission can lapse between browser
   sessions even though the handle itself still exists — reading through
   a lapsed handle throws NotAllowedError. This re-requests permission
   (must be called from a user gesture, e.g. tapping Play) before
   reading, so a handle-based track keeps working across app restarts
   instead of silently failing. */
async function getFileWithPermission(handle) {
  try {
    if ((await handle.queryPermission({ mode: 'read' })) !== 'granted') {
      const result = await handle.requestPermission({ mode: 'read' });
      if (result !== 'granted') throw new Error('Permission to access this file was not granted.');
    }
  } catch (e) {
    // queryPermission/requestPermission aren't implemented on every
    // handle-producing browser path — if they're missing, fall through
    // and just try getFile() directly rather than blocking playback.
  }
  return handle.getFile();
}
MS.fileFromTrack=fileFromTrack;

/* ── Android-safe import (works with plain <input type="file">, no
   File System Access API required) ──
   preservePath=true keeps webkitRelativePath so folder structure survives
   (used by the DJ browser's folder grouping). */
async function importFileList(fileList, preservePath=false) {
  const files = [...fileList].filter(f => audioExt.has(getExt(f.name)));
  if (!files.length) { MS.toast('No audio files found in that selection.','warn'); return; }
  let count=0;
  for (const file of files) {
    const relPath = preservePath && file.webkitRelativePath ? file.webkitRelativePath : file.name;
    const id = fingerprint(file, relPath);
    ...
    if (!track) track = {
      ...
      lastModified:file.lastModified, type:file.type, artwork:null, source:'blob'
    };
    track.path=relPath; track.size=file.size;
    await dbPut('tracks', track);
    await dbPut('trackBlobs', { id, blob:file });
    count++;
  }
  MS.library = await dbAll('tracks');
  MS.emit('library:updated', MS.library);
  MS.toast(`Imported ${count} song${count===1?'':'s'}.`,'ok');
  return count;
}
MS.importFileList = importFileList;
